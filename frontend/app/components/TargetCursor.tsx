...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const moveHandler = (e: MouseEvent) => {
      // Check if cursor is within the scope
      const scopeElement = document.querySelector(scopeSelector);
      if (!scopeElement) return;
      
      const rect = scopeElement.getBoundingClientRect();
      const isInScope = e.clientX >= rect.left && e.clientX <= rect.right && 
                       e.clientY >= rect.top && e.clientY <= rect.bottom;
      
      // Also check if cursor is over the header (which should always use default cursor)
      const header = document.querySelector('header');
      let isOverHeader = false;
      if (header) {
        const headerRect = header.getBoundingClientRect();
        isOverHeader = e.clientX >= headerRect.left && e.clientX <= headerRect.right && 
                      e.clientY >= headerRect.top && e.clientY <= headerRect.bottom;
      }
      
      if (isInScope && !isOverHeader) {
        ...
        // Restore default cursor when outside scope or over header
    ...
    const scrollHandler = () => {
      ...
      // Check if cursor is still in scope after scroll
      const scopeElement = document.querySelector(scopeSelector);
      if (scopeElement) {
        const rect = scopeElement.getBoundingClientRect();
        const isInScope = mouseX >= rect.left && mouseX <= rect.right && 
    ...
    //---------------------------------------------------------------
        ...
        const mouseDownHandler = ():void => {
          // Check if cursor is within scope before applying effects
          const scopeElement = document.querySelector(scopeSelector);
          if (!scopeElement) return;
          ...
          // Also check if cursor is over the header
          const header = document.querySelector('header');
          let isOverHeader = false;
          if (header) {
            const headerRect = header.getBoundingClientRect();
            isOverHeader = mouseX >= headerRect.left && mouseX <= headerRect.right && 
                          mouseY >= headerRect.top && mouseY <= headerRect.bottom;
          }
          
          if (!isInScope || isOverHeader) return;
    ...
    const enterHandler = (e: MouseEvent) => {
      // Check if the event is within the scope
      const scopeElement = document.querySelector(scopeSelector);
      if (!scopeElement) return;
